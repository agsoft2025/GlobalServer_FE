...
import { getAdmins, deleteAdmin, updateAdmin, createAdmin, type Admin, type AdminQueryParams } from "../api/service/adminService";
...
const SchoolAdmins = () => {
    ...
    const handleSubmitAdmin = async (data: any) => {
        try {
            ...
            console.error(error);
            throw error; // Re-throw so dialog can catch and show error
        }
    };
                    ...
                    <TableBody>
                        {admins.map((admin) => (
                            <TableRow key={admin._id}>
                                ...
                                <TableCell>{admin.location?.location_name || "N/A"}</TableCell>
