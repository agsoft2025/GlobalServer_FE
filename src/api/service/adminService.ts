...
export interface Admin {
  ...
  role: "ADMIN" | "SUPER ADMIN"; // Backend says "SUPER ADMIN"
  isDeleted: boolean;
  createdAt: string;
  updatedAt: string;
}
...
export async function createAdmin(data: any): Promise<{ success: boolean; data: Admin }> {
...
export async function getLocations(): Promise<{ success: boolean; data: Location[] }> {
  const res = await schoolApi.get("/location");
  return res.data;
}